...
pub fn hiffy_call(
    ...
    // ensure we can call the requested op
    check_op(op)?;

    // ensure the provided leases meet the needs of the op
...
fn check_op(op: &idol::IdolOperation) -> Result<()> {
    ...
    match op.operation.leases.len() {
        ...
        0 => (),
        // if the op requires a single lease it must be either `read` or `write
        1 => {
