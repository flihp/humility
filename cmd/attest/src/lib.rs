...
//! ## `humility attest`
//!
//! Communicate with the RoT-R task
//!
//! ```console
//! $ humility attest record \
...
#[clap(name = "subcmd")]
enum AttestCmd {
    /// Send a hash to the RoT-R to be recorded in the measurement log.
    Record { hash: String },
    ...
    /// Get number of certs in cert chain from Alias cert to mfg intermediate
    CertChainLen,
    /// Get the length of cert in cert chain
    CertLen {
...
impl<'a> AttestHandler<'a> {
    ...
    /// Send a hash / measurement value to attest task for recording in the
    /// measurement log.
    pub fn record(&mut self, hash: String) -> Result<()> {
        let cmd = self.hubris.get_idol_command("Attest.record")?;

        if hash.len() != 64 {
            humility::msg!("hex string must be 64 characters / 32 bytes");
        }

        let hash = decode_hex(hash)?;

        humility::msg!("recording hash: {:?}", hash);
...
fn decode_hex(hex: String) -> Result<Vec<u8>> {
    // Adding a dependency on the hex crate seemed like overkill.
    (0..hex.len())
        .step_by(2)
...
fn attest(context: &mut ExecutionContext) -> Result<()> {
    ...
    match subargs.cmd {
        AttestCmd::Record { hash } => {
            humility::msg!("recording measurement ...");
            handler.record(hash)
        }
